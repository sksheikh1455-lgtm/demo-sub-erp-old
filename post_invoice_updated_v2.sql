...
AS $function$
            ...
            BEGIN
                ...
                VALUES (v_journal_id, v_effective_company_id, v_invoice.date, COALESCE(v_invoice.invoice_number, v_invoice.id), 'Invoice ' || COALESCE(v_invoice.invoice_number, v_invoice.id), 'DRAFT', v_invoice.customer_id, 'INV', jsonb_build_object('source', 'INV', 'journalEntryId', v_journal_id));
                ...
                END IF;
                SELECT id INTO v_cogs_acc FROM docs_accounts WHERE (code IN ('5011', '500100', '500101', 'COGS') OR data->>'code' IN ('5011', '500100', '500101', 'COGS')) AND company_id = v_effective_company_id LIMIT 1;
                IF v_cogs_acc IS NULL THEN SELECT id INTO v_cogs_acc FROM docs_accounts WHERE (name ILIKE '%cost of goods%' OR data->>'name' ILIKE '%cost of goods%') AND company_id = v_effective_company_id LIMIT 1; END IF;
                IF v_cogs_acc IS NULL THEN SELECT id INTO v_cogs_acc FROM docs_accounts WHERE (type = 'EXPENSE' OR data->>'type' = 'EXPENSE') AND company_id = v_effective_company_id LIMIT 1; END IF;
                IF v_cogs_acc IS NULL THEN
                    v_cogs_acc := 'acc-cogs-' || v_effective_company_id;
                    INSERT INTO docs_accounts (id, company_id, code, name, type, data) VALUES (v_cogs_acc, v_effective_company_id, 'COGS', 'Cost of Goods Sold', 'EXPENSE', '{"code":"COGS","name":"Cost of Goods Sold","type":"EXPENSE"}') ON CONFLICT DO NOTHING;
                END IF;

                SELECT id INTO v_inv_acc FROM docs_accounts WHERE (code IN ('1013', '100500', '100501', 'INVENTORY') OR data->>'code' IN ('1013', '100500', '100501', 'INVENTORY')) AND company_id = v_effective_company_id LIMIT 1;
                IF v_inv_acc IS NULL THEN SELECT id INTO v_inv_acc FROM docs_accounts WHERE (name ILIKE '%inventory%' OR data->>'name' ILIKE '%inventory%') AND company_id = v_effective_company_id LIMIT 1; END IF;
                IF v_inv_acc IS NULL THEN SELECT id INTO v_inv_acc FROM docs_accounts WHERE (type = 'ASSET' OR data->>'type' = 'ASSET') AND company_id = v_effective_company_id LIMIT 1; END IF;
                IF v_inv_acc IS NULL THEN
                    v_inv_acc := 'acc-inv-' || v_effective_company_id;
                    INSERT INTO docs_accounts (id, company_id, code, name, type, data) VALUES (v_inv_acc, v_effective_company_id, 'INV', 'Inventory Asset', 'ASSET', '{"code":"INV","name":"Inventory Asset","type":"ASSET"}') ON CONFLICT DO NOTHING;
                END IF;
                    ...
                    LOOP
                        ...
                        IF v_item->>'type' = 'PRODUCT' OR v_item->>'type' IS NULL THEN
                            ...
                            IF v_item->>'type' = 'PRODUCT' OR v_item->>'type' IS NULL THEN
                                ...
                                IF (v_item->>'quantity')::numeric > 0 AND COALESCE(v_product_record.track_inventory, COALESCE((v_product_record.data->>'trackInventory')::boolean, true)) THEN
                                    v_cogs_amount := ROUND(v_wac_cost * (v_item->>'quantity')::numeric, 2);
                                    IF v_cogs_amount > 0 THEN
                                        INSERT INTO docs_journal_lines (id, journal_id, company_id, account_id, debit, credit, description)
                                        VALUES ('JL-' || v_journal_id || '-cogs-' || v_idx, v_journal_id, v_effective_company_id, COALESCE(v_cogs_acc, 'MISSING-COGS'), v_cogs_amount, 0, 'Cost of Goods Sold: ' || COALESCE(v_item->>'description', v_product_record.name));
                                        v_total_debit := v_total_debit + v_cogs_amount;

                                        INSERT INTO docs_journal_lines (id, journal_id, company_id, account_id, debit, credit, description)
                                        VALUES ('JL-' || v_journal_id || '-inv-' || v_idx, v_journal_id, v_effective_company_id, COALESCE(v_inv_acc, 'MISSING-INV'), 0, v_cogs_amount, 'Inventory Asset: ' || COALESCE(v_item->>'description', v_product_record.name));
                                        v_total_credit := v_total_credit + v_cogs_amount;
                                    END IF;
                                END IF;
