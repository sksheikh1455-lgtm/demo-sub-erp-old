...
async function run() {
  const { data, error } = await supabase.rpc('get_general_ledger', {
     ...
     p_partner_ids: ['3d106296-a1a3-4f56-8ec8-66089bb9d2b7'], // SUL contact id from DB lines
  ...
  const entryRaju = mapped.filter(r => (r.reference && r.reference.includes('1175')) || (r.description && r.description.includes('1175')) || (r.journal_id && r.journal_id.includes('1175')) || (r.journal_id && r.journal_id.includes('245073BC')));
  ...
  console.log("Original matched entries:");
  console.log(entryRaju.map(e => e.journal_id));
  console.log("Filtered matched entries:");
  console.log(entryFiltered.map(e => e.journal_id));
}
run();
