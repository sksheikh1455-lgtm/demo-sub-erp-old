import { createClient } from '@supabase/supabase-js';
const supabase = createClient(
  "https://buspgzsamhfmjrmmwpmo.supabase.co",
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ1c3BnenNhbWhmbWpybW13cG1vIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzczMzgxMDgsImV4cCI6MjA5MjkxNDEwOH0.8Pj-NoDqlenxJr2azDs5L-gCfPJ-Bvcdzalq5UqKcRM"
);
async function run() {
  let all = [];
  let page = 0;
  while(true) {
    const {data: pageData} = await supabase.from('docs_inventory_transactions').select('*').range(page*1000, (page+1)*1000-1);
    if (!pageData || pageData.length === 0) break;
    all.push(...pageData);
    page++;
  }
  const found = all.filter(d => JSON.stringify(d).toLowerCase().includes('4core'));
  console.log("Found in tx:", found.length);
}
run();
