...
BEGIN
    ...
    WITH raw_data AS (
        SELECT 
            p_start_date::date AS transaction_date,
            'OPENING_BALANCE'::text AS type,
            ''::text AS invoice_bill_num,
            ...
            v_opening_balance::NUMERIC AS running_balance,
            0 AS group_order
        ...
        SELECT 
            ...
            ('Invoice ' || COALESCE(i.invoice_number, i.id))::text AS narration,
            ...
            COALESCE((SELECT SUM(jl.debit - jl.credit) 
             FROM docs_journal_lines jl 
             JOIN docs_accounts a ON a.id = jl.account_id 
             WHERE jl.journal_id = COALESCE(i.journal_entry_id, i.data->>'journalEntryId', i.data->>'journal_entry_id') AND a.code = '100100'), 0)::numeric AS cash_impact,
        ...
        SELECT 
            ...
            COALESCE((SELECT SUM(jl.debit - jl.credit) 
             FROM docs_journal_lines jl 
             JOIN docs_accounts a ON a.id = jl.account_id 
             WHERE jl.journal_id = COALESCE(cn.data->>'journalEntryId', cn.data->>'journal_entry_id') AND a.code = '100100'), 0)::numeric AS cash_impact,
            0::NUMERIC AS running_balance,
            2 AS group_order
        ...
        SELECT 
            ...
            COALESCE((SELECT SUM(jl.debit - jl.credit) 
             FROM docs_journal_lines jl 
             JOIN docs_accounts a ON a.id = jl.account_id 
             WHERE jl.journal_id = COALESCE(b.journal_entry_id, b.data->>'journalEntryId', b.data->>'journal_entry_id') AND a.code = '100100'), 0)::numeric AS cash_impact,
            0::NUMERIC AS running_balance,
            3 AS group_order
        FROM docs_bills b
        ...
        WHERE b.company_id = p_company_id 
          ...
          AND b.status IN ('POSTED', 'PAID', 'PARTIAL', 'FULL_REFUNDED', 'PARTIAL_REFUNDED')
        ...
        SELECT 
            ...
            (CASE
                WHEN p.type = 'REFUND' THEN 'Refund ' || COALESCE((SELECT c.name FROM docs_contacts c WHERE c.id = p.contact_id LIMIT 1), 'Unknown') || ' ' || REPLACE(COALESCE(p.payment_number, p.id), 'REFUND-', '')
                WHEN (
            ...
            COALESCE(p.amount, 0)::numeric AS paid,
            0::NUMERIC AS due,
            COALESCE((SELECT SUM(jl.debit - jl.credit) 
             FROM docs_journal_lines jl 
             JOIN docs_accounts a ON a.id = jl.account_id 
             WHERE jl.journal_id = COALESCE(p.data->>'journalEntryId', p.data->>'journal_entry_id', (SELECT id FROM docs_journals tj WHERE tj.id IN ('JE-CPAY-' || REPLACE(UPPER(p.id), 'PAY-', ''), 'JE-VPAY-' || REPLACE(UPPER(p.id), 'PAY-', ''), 'JE-PAY-' || REPLACE(UPPER(p.id), 'PAY-', ''), 'JE-' || REPLACE(UPPER(p.id), 'PAY-', '')) LIMIT 1)) AND a.code = '100100'), 0)::numeric AS cash_impact,
            0::NUMERIC AS running_balance,
            4 AS group_order
        ...
        SELECT 
            ...
            ABS(COALESCE(SUM(jl.debit - jl.credit), 0))::numeric AS amount,
            ABS(COALESCE(SUM(jl.debit - jl.credit), 0))::numeric AS paid,
            0::NUMERIC AS due,
        ...
        WHERE j.company_id = p_company_id
          ...
          AND a.code = '100100'
          AND j.id NOT IN (
              SELECT COALESCE(journal_entry_id, data->>'journalEntryId', data->>'journal_entry_id', '') FROM docs_invoices WHERE company_id = p_company_id AND status IN ('POSTED', 'PAID', 'PARTIAL', 'FULL_REFUNDED', 'PARTIAL_REFUNDED')
              UNION ALL
              SELECT COALESCE(data->>'journalEntryId', data->>'journal_entry_id', '') FROM docs_credit_notes WHERE company_id = p_company_id AND status IN ('POSTED', 'CLOSED')
              UNION ALL
              SELECT COALESCE(journal_entry_id, data->>'journalEntryId', data->>'journal_entry_id') FROM docs_bills WHERE company_id = p_company_id AND status IN ('POSTED', 'PAID', 'PARTIAL', 'FULL_REFUNDED', 'PARTIAL_REFUNDED') AND COALESCE(journal_entry_id, data->>'journalEntryId', data->>'journal_entry_id') IS NOT NULL
              UNION ALL
              SELECT COALESCE(data->>'journalEntryId', data->>'journal_entry_id', (SELECT id FROM docs_journals tj WHERE tj.id IN ('JE-CPAY-' || REPLACE(UPPER(p.id), 'PAY-', ''), 'JE-VPAY-' || REPLACE(UPPER(p.id), 'PAY-', ''), 'JE-PAY-' || REPLACE(UPPER(p.id), 'PAY-', ''), 'JE-' || REPLACE(UPPER(p.id), 'PAY-', '')) LIMIT 1)) FROM docs_payments p WHERE p.company_id = p_company_id AND p.status = 'POSTED'
          )
        GROUP BY j.id, j.date, j.journal_number
    )
    SELECT 
        rd.transaction_date::date, 
        rd.type::text, 
        rd.invoice_bill_num::text, 
        rd.narration::text, 
        rd.partner::text, 
        rd."user"::text, 
        rd.amount::numeric, 
        rd.paid::numeric, 
        rd.due::numeric, 
        rd.cash_impact::numeric, 
        rd.running_balance::numeric AS balance 
    FROM raw_data rd ORDER BY rd.transaction_date, rd.group_order, rd.invoice_bill_num;
END;
$$;
`;
...
async function main() {
  ...
  console.log('Resulting SUM from final function:', fnRes2.rows);
