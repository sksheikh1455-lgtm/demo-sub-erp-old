    ...
    WITH raw_data AS (
        ...
        SELECT 
            ...
            COALESCE((SELECT SUM(jl.debit - jl.credit) 
             FROM docs_journal_lines jl 
             JOIN docs_accounts a ON a.id = jl.account_id 
             WHERE jl.journal_id = i.data->>'journalEntryId' AND a.code = '100100'), 0) AS cash_impact,
        ...
        SELECT 
            ...
            CASE WHEN (
                ...
                ELSE 'Misc Settlement ' || COALESCE(p.payment_number, p.id) END AS narration,
            
            COALESCE((
               SELECT c.name 
               FROM docs_contacts c 
               WHERE c.id = p.customer_id OR c.id = p.vendor_id
               LIMIT 1
            ), 'Unknown') AS partner,
            ...
            COALESCE((SELECT SUM(jl.debit - jl.credit) 
             FROM docs_journal_lines jl 
             JOIN docs_accounts a ON a.id = jl.account_id 
             WHERE jl.journal_id = p.data->>'journalEntryId' AND a.code = '100100'), 0) AS cash_impact,
            0::NUMERIC AS running_balance,
            4 AS group_order
        ...
        SELECT 
            ...
            'JEN' AS type,
            COALESCE(j.journal_number, j.reference_number, j.id) AS invoice_bill_num,
        ...
        GROUP BY j.id, j.date, j.journal_number, j.reference_number
    )
    SELECT * FROM raw_data ORDER BY transaction_date, invoice_bill_num;
