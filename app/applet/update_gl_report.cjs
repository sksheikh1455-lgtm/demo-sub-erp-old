    ...
    WITH raw_data AS (
        ...
        SELECT 
            ...
            COALESCE(
              ...
              (SELECT c_inner.name FROM docs_invoices i2 LEFT JOIN docs_contacts c_inner ON i2.customer_id = c_inner.id WHERE j.id = 'JE-CPAY-' || UPPER(REPLACE(REPLACE('PAY-AUTO-' || i2.id, 'PAY-', ''), 'PAY-', '')) LIMIT 1),
              'Various'
            ) AS partner,
