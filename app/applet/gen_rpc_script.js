...
AS $$
            ...
            BEGIN
                ...
                VALUES ('JL-' || v_journal_id || '-ar', v_journal_id, v_effective_company_id, 
                        (SELECT id FROM docs_accounts WHERE code IN ('1012','100200','100201','AR') AND company_id = v_effective_company_id LIMIT 1),
...
async function run() {
  ...
  await c.connect();
  await c.query(\`${sql}\`);
  console.log("Updated RPC function post_invoice");
  process.exit();
}
run();
`);
