...
AS $function$
    ...
    BEGIN
      ...
      IF TG_TABLE_NAME = 'docs_bills' AND (v_is_posted AND (TG_OP = 'INSERT' OR v_status_old IS NULL OR v_status_old NOT IN ('POSTED', 'PAID', 'PARTIAL', 'IN_PAYMENT', 'OPEN', 'ACTIVE', 'CLOSED'))) THEN
        -- Calculate proportional discount factor: (subtotal - discount_total) / subtotal
        IF COALESCE(NEW.subtotal, 0) > 0 THEN
           v_bill_discount_factor := (NEW.subtotal - COALESCE(NEW.discount_total, 0)) / NEW.subtotal;
        ELSE
    ...
    $function$;
    `);
    console.log('SUCCESS: generate_inventory_movements updated for vendor bill global discount factor!');
  } catch (e: any) {
