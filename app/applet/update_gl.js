...
BEGIN
    ...
    WITH raw_data AS (
        ...
        SELECT 
            ...
            (COALESCE(i.total, 0) - CASE WHEN i.status IN ('PAID', 'FULL_REFUNDED') THEN 0 ELSE COALESCE((i.data->>'due')::numeric, i.total) END) AS paid,
            CASE WHEN i.status IN ('PAID', 'FULL_REFUNDED') THEN 0 ELSE COALESCE((i.data->>'due')::numeric, i.total) END AS due,
        ...
        SELECT 
            ...
            CASE WHEN cn.status = 'CLOSED' THEN COALESCE(cn.total, 0) ELSE COALESCE(cn.total, 0) - COALESCE((cn.data->>'due')::numeric, cn.total) END AS paid,
            CASE WHEN cn.status = 'CLOSED' THEN 0 ELSE COALESCE((cn.data->>'due')::numeric, cn.total) END AS due,
            COALESCE((SELECT SUM(jl.debit - jl.credit) 
             FROM docs_journal_lines jl 
             JOIN docs_accounts a ON a.id = jl.account_id 
        ...
        SELECT 
            ...
            (COALESCE(b.total, 0) - CASE WHEN b.status = 'PAID' THEN 0 ELSE COALESCE((b.data->>'due')::numeric, b.total) END) AS paid,
            CASE WHEN b.status = 'PAID' THEN 0 ELSE COALESCE((b.data->>'due')::numeric, b.total) END AS due,
            COALESCE((SELECT SUM(jl.debit - jl.credit) 
             FROM docs_journal_lines jl 
             JOIN docs_accounts a ON a.id = jl.account_id 
        ...
        SELECT 
            ...
            COALESCE(j.data->>'narration', j.notes, 'Journal Entry') AS narration,
            '---' AS partner,
            COALESCE(j.data->>'preparedBy', 'System') AS "user",
            j.total_debit AS amount,
            j.total_debit AS paid,
            0::NUMERIC AS due,
            COALESCE((SELECT SUM(jl.debit - jl.credit) 
             FROM docs_journal_lines jl 
             JOIN docs_accounts a ON a.id = jl.account_id 
        ...
        WHERE j.company_id = p_company_id 
          ...
          AND NOT EXISTS (
              SELECT 1 FROM docs_invoices WHERE COALESCE(journal_entry_id, data->>'journalEntryId') = j.id AND status IN ('POSTED', 'PAID', 'PARTIAL', 'FULL_REFUNDED', 'PARTIAL_REFUNDED')
          )
          AND NOT EXISTS (
              SELECT 1 FROM docs_bills WHERE COALESCE(journal_entry_id, data->>'journalEntryId') = j.id AND status IN ('POSTED', 'PAID', 'PARTIAL')
          )
          AND NOT EXISTS (
              SELECT 1 FROM docs_credit_notes WHERE data->>'journalEntryId' = j.id AND status IN ('POSTED', 'CLOSED')
          )
          AND NOT EXISTS (
              SELECT 1 FROM docs_payments WHERE COALESCE(data->>'journalEntryId', ('JE-' || CASE WHEN type IN ('RECEIPT','COLLECTION','REFUND') THEN 'CPAY' ELSE 'VPAY' END || '-' || replace(replace(UPPER(id), 'PAY-', ''), 'PAY-', ''))) = j.id AND status = 'POSTED'
          )
          AND NOT EXISTS (
              SELECT 1 FROM docs_payments WHERE COALESCE(data->>'journalEntryId', ('JE-' || CASE WHEN type IN ('RECEIPT','COLLECTION','REFUND') THEN 'CPAY' ELSE 'VPAY' END || '-' || replace(replace(UPPER(id), 'PAY-', ''), 'PAY-', ''))) = j.id AND status != 'POSTED'
          )
    ...
    ordered_data AS (
        SELECT *,
               SUM(raw_data.cash_impact) OVER (ORDER BY raw_data.group_order, NULLIF(regexp_replace(raw_data.invoice_bill_num, '[^0-9]', '', 'g'), '')::numeric, raw_data.transaction_date ROWS UNBOUNDED PRECEDING) AS computed_balance
        FROM raw_data
    )
    SELECT 
    ...
    FROM ordered_data
    ORDER BY ordered_data.group_order, NULLIF(regexp_replace(ordered_data.invoice_bill_num, '[^0-9]', '', 'g'), '')::numeric, ordered_data.transaction_date;
END;
$$;
`;
