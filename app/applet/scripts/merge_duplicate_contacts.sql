...
DECLARE
    ...
    v_ln_cnt INT;
BEGIN
    -- Disable the historical double-entry validation trigger on journal lines 
    -- to allow bulk updates on old entries without triggering strict integrity checks
    ...
    -- Note: If you want to merge globally across different companies, remove 'company_id' from the GROUP BY.
    FOR v_group IN (
        ...
        -- 1. Identify the Master ID for this group
        -- Keeping the one with the most combined invoices and bills.
        ...
        IF v_master_id IS NOT NULL THEN
            ...
            FOR v_dup_id IN (
                ...
                -- J. Delete the now orphaned duplicate contact row from docs_contacts 
                -- (will cascade to docs_contact_companies due to foreign key)
    ...
    -- Re-enable the strict double entry check trigger
...
EXCEPTION WHEN OTHERS THEN
    -- Ensure trigger is safely re-enabled if any error occurs
    ALTER TABLE public.docs_journal_lines ENABLE TRIGGER trg_strict_double_entry_check;
    RAISE;
END;
$$;
