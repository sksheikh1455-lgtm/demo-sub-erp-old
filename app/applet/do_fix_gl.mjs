...
BEGIN
    ...
    WITH raw_data AS (
        ...
        SELECT 
            ...
            (CASE
                WHEN p.type = 'REFUND' THEN 'Refund ' || COALESCE((SELECT c.name FROM docs_contacts c WHERE c.id = p.contact_id LIMIT 1), 'Unknown') || ' ' || REGEXP_REPLACE(COALESCE(p.payment_number, p.id), '^(RECEIPT-|REFUND-|PAYMENT-)?(PAY-)', '\\2')
                WHEN (
        ...
        WHERE j.company_id = $1
          ...
          AND a.code = '100100'
          AND j.id NOT IN (
              SELECT COALESCE(journal_entry_id, data->>'journalEntryId', data->>'journal_entry_id', '') FROM docs_invoices WHERE company_id = $1 AND status IN ('POSTED', 'PAID', 'PARTIAL', 'FULL_REFUNDED', 'PARTIAL_REFUNDED')
              UNION ALL
              SELECT COALESCE(data->>'journalEntryId', data->>'journal_entry_id', '') FROM docs_credit_notes WHERE company_id = $1 AND status IN ('POSTED', 'CLOSED')
              UNION ALL
...
async function main() {
  ...
  console.log('SQL Executed with SECURITY DEFINER & EXECUTE.');
