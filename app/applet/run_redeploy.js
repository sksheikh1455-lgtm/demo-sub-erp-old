...
CREATE OR REPLACE FUNCTION public.get_general_ledger_report(p_company_id text, p_start_date date, p_end_date date)
     ...
     WITH raw_data AS (
         ...
         SELECT 
             ...
             COALESCE((SELECT SUM(jl.debit - jl.credit) 
              FROM docs_journal_lines jl 
              JOIN docs_accounts a ON a.id = jl.account_id 
              WHERE jl.journal_id = COALESCE(p.data->>'journalEntryId', p.data->>'journal_entry_id', 'JE-CPAY-' || REPLACE(UPPER(p.id), 'PAY-', '')) AND a.code = '100100'), 0)::numeric AS cash_impact,
             0::NUMERIC AS running_balance,
             4 AS group_order
         ...
         WHERE j.company_id = $1
           ...
           AND a.code = '100100'
           AND j.id NOT IN (
               SELECT COALESCE(journal_entry_id, data->>'journalEntryId', data->>'journal_entry_id', '') FROM docs_invoices WHERE company_id = $1
               UNION ALL
               SELECT COALESCE(data->>'journalEntryId', data->>'journal_entry_id', '') FROM docs_credit_notes WHERE company_id = $1
               UNION ALL
               SELECT COALESCE(journal_entry_id, data->>'journalEntryId', data->>'journal_entry_id') FROM docs_bills WHERE company_id = $1 AND COALESCE(journal_entry_id, data->>'journalEntryId', data->>'journal_entry_id') IS NOT NULL
               UNION ALL
               SELECT COALESCE(data->>'journalEntryId', data->>'journal_entry_id', 'JE-CPAY-' || REPLACE(UPPER(id), 'PAY-', '')) FROM docs_payments WHERE company_id = $1
               UNION ALL
               SELECT 'JE-VPAY-' || REPLACE(UPPER(id), 'PAY-', '') FROM docs_payments WHERE company_id = $1
               UNION ALL
               SELECT 'JE-PAY-' || REPLACE(UPPER(id), 'PAY-', '') FROM docs_payments WHERE company_id = $1
               UNION ALL
               SELECT 'JE-' || REPLACE(UPPER(id), 'PAY-', '') FROM docs_payments WHERE company_id = $1
           )
 ...
 $function$;
  ...
  try {
      ...
      console.log('SUCCESS executed inline redeploy_rpc');
  } catch(e) {
