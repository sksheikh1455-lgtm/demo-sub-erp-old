...
async function run() {
  ...
  for (const m of mismatches) {
    ...
    // In pg parameterized queries, you can just treat JSON strings as text and let PG cast it: CAST($1 AS jsonb)
    ...
    await c.query(`
      ...
      SET applied_invoices = CAST($1 AS jsonb), 
          data = jsonb_set(data, '{appliedInvoices}', CAST($1 AS jsonb)) 
      WHERE id = $2
    `, [arr, m.payment_id]);
