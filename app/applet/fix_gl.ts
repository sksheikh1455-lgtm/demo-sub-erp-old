...
async function main() {
  ...
  def = def.split("SELECT COALESCE(journal_entry_id, data->>'journalEntryId', data->>'journal_entry_id', '') FROM docs_invoices WHERE company_id = $1").join(
  ...
  def = def.split("SELECT COALESCE(data->>'journalEntryId', data->>'journal_entry_id', '') FROM docs_credit_notes WHERE company_id = $1").join(
  ...
  def = def.split("SELECT COALESCE(journal_entry_id, data->>'journalEntryId', data->>'journal_entry_id') FROM docs_bills WHERE company_id = $1 AND COALESCE(journal_entry_id, data->>'journalEntryId', data->>'journal_entry_id') IS NOT NULL").join(
  ...
  // Remove the old payments NOT IN lines
  const oldPaymentsLines = `UNION ALL
  ...
  def = def.split(oldPaymentsLines).join(
