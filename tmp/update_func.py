...
with open('/tmp/func_out.txt', 'r') as f:
    text = f.read()

text = re.sub(
    r"AND i\.status IN \('POSTED', 'PAID', 'PARTIAL', 'FULL_REFUNDED', 'PARTIAL_REFUNDED'\)",
    r"-- removed status filter for invoices to show all sequence",
    text
)

text = re.sub(
    r"COALESCE\(i\.total, 0\) AS amount",
    r"CASE WHEN i.status NOT IN ('POSTED', 'PAID', 'PARTIAL', 'FULL_REFUNDED', 'PARTIAL_REFUNDED') THEN 0 ELSE COALESCE(i.total, 0) END AS amount",
    text
)

# For narration, paid, due, cash_impact
# Let's just use string replace since regex might fail on multiline
narration_part = """COALESCE(
...
new_narration_part = """CASE WHEN i.status NOT IN ('POSTED', 'PAID', 'PARTIAL', 'FULL_REFUNDED', 'PARTIAL_REFUNDED') THEN 'MISSING / ' || UPPER(i.status) ELSE COALESCE(
               (SELECT 'Paid via ' || string_agg(p.payment_number, ', ')
                FROM docs_payments p
                WHERE p.id = 'PAY-AUTO-' || i.id
                   OR EXISTS (
...
new_paid_part = """CASE WHEN i.status NOT IN ('POSTED', 'PAID', 'PARTIAL', 'FULL_REFUNDED', 'PARTIAL_REFUNDED') THEN 0 ELSE CASE 
                WHEN COALESCE(c.name, 'Unknown') ILIKE '%Cash Sale%' THEN COALESCE(i.total, 0)
                ELSE (COALESCE(i.total, 0) - CASE WHEN i.status IN ('PAID', 'FULL_REFUNDED') THEN 0 ELSE COALESCE((i.data->>'due')::numeric, i.total) END)
            END END AS paid"""
...
new_due_part = """CASE WHEN i.status NOT IN ('POSTED', 'PAID', 'PARTIAL', 'FULL_REFUNDED', 'PARTIAL_REFUNDED') THEN 0 ELSE CASE 
                WHEN COALESCE(c.name, 'Unknown') ILIKE '%Cash Sale%' THEN 0
                WHEN i.status IN ('PAID', 'FULL_REFUNDED') THEN 0 
                ELSE COALESCE((i.data->>'due')::numeric, i.total) 
            END END AS due"""
...
cash_impact_part = """COALESCE((SELECT SUM(jl.debit - jl.credit) 
...
new_cash_impact_part = """CASE WHEN i.status NOT IN ('POSTED', 'PAID', 'PARTIAL', 'FULL_REFUNDED', 'PARTIAL_REFUNDED') THEN 0 ELSE COALESCE((SELECT SUM(jl.debit - jl.credit) 
             FROM docs_journal_lines jl 
             JOIN docs_accounts a ON a.id = jl.account_id 
...
with open('/tmp/func_updated.sql', 'w') as f:
    f.write(text)
