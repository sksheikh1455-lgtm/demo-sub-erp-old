...
BEGIN
    ...
    v_partner_code := CASE 
      WHEN v_is_receipt AND NOT v_is_refund THEN '100201' 
      WHEN v_is_refund AND v_is_receipt THEN '200101' 
      WHEN v_is_refund AND NOT v_is_receipt THEN '100201' 
      ELSE '200101' 
    END;

    v_partner_acc := COALESCE(
      (SELECT id FROM docs_accounts WHERE code = v_partner_code AND company_id = v_effective_company_id LIMIT 1),
      (SELECT id FROM docs_accounts WHERE code = v_partner_code LIMIT 1)
    );
    ...
    ELSE 
       v_liquidity_acc := COALESCE(
         (SELECT id FROM docs_accounts WHERE (code = '100100' OR type IN ('CASH', 'BANK')) AND company_id = v_effective_company_id LIMIT 1),
         (SELECT id FROM docs_accounts WHERE code = '100100' OR type IN ('CASH', 'BANK') LIMIT 1)
       );
    END IF;

    IF v_partner_acc IS NULL OR v_liquidity_acc IS NULL THEN
        RAISE EXCEPTION 'CONFIRMATION FAILED: A REQUIRED ACCOUNT (PARTNER/LIQUIDITY) COULD NOT BE RESOLVED PARTNER: %, LIQUIDITY: %', v_partner_acc, v_liquidity_acc;
    END IF;
    ...
    -- Update with FULL JSON document AND set status to POSTED
    ...
    BEGIN
        IF (v_payment.data ? 'appliedInvoices') THEN
            FOR v_alloc IN SELECT * FROM jsonb_array_elements(v_payment.data->'appliedInvoices') LOOP
        ...
        IF (v_payment.data ? 'appliedBills') THEN
            FOR v_alloc IN SELECT * FROM jsonb_array_elements(v_payment.data->'appliedBills') LOOP
