DROP FUNCTION IF EXISTS get_partner_summary(text[],text,date);

CREATE OR REPLACE FUNCTION public.get_partner_summary(p_company_ids text[], p_contact_type text, p_as_of_date date)
...
BEGIN
    ...
    journal_sums AS (
        SELECT 
            dl1.effective_contact_id,
            dl1.company_id,
            COALESCE(SUM(dl1.amount), 0) AS tx_bal,
            EXISTS (
                SELECT 1 
                FROM derived_lines dl2
                WHERE dl2.journal_type = 'OPENING_BALANCE'
                  AND dl2.company_id = dl1.company_id
                  AND dl2.effective_contact_id = dl1.effective_contact_id
            ) AS has_op_journal
        FROM derived_lines dl1
        GROUP BY dl1.effective_contact_id, dl1.company_id
    )
    SELECT 
    ...
    LEFT JOIN journal_sums js ON cc.cid = js.effective_contact_id AND cc.company_id = js.company_id
