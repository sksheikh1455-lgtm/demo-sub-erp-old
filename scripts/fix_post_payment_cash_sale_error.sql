...
BEGIN
    ...
    IF v_is_receipt AND v_payment.applied_invoices IS NOT NULL THEN
        FOR v_alloc IN SELECT * FROM jsonb_array_elements(CASE WHEN jsonb_typeof(v_payment.applied_invoices) = 'array' THEN v_payment.applied_invoices ELSE '[]'::jsonb END) LOOP
            ...
            IF FOUND THEN
                ...
                SET status = CASE WHEN v_new_amt_paid >= COALESCE(total, (data->>'total')::numeric, 0) - 0.01 THEN 'PAID' ELSE 'PARTIAL' END,
                    updated_at = NOW()
    ...
    ELSIF NOT v_is_receipt AND v_payment.applied_bills IS NOT NULL THEN
        FOR v_alloc IN SELECT * FROM jsonb_array_elements(CASE WHEN jsonb_typeof(v_payment.applied_bills) = 'array' THEN v_payment.applied_bills ELSE '[]'::jsonb END) LOOP
            ...
            IF FOUND THEN
                SELECT COALESCE(SUM((al->>'amount')::numeric), 0) INTO v_new_amt_paid
                FROM docs_payments p, jsonb_array_elements(CASE WHEN jsonb_typeof(p.applied_bills) = 'array' THEN p.applied_bills ELSE '[]'::jsonb END) al
