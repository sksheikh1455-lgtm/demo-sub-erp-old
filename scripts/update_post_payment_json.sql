...
BEGIN
    ...
    v_prepared_by := COALESCE(v_payment.data->>'preparedBy', v_payment.data->>'salesperson', 'System');
    v_created_by_id := NULLIF(TRIM(v_payment.data->>'createdById'), '');
    IF v_created_by_id IS NULL THEN
        v_created_by_id := NULLIF(TRIM(v_payment.data->>'authorId'), '');
    END IF;
    ...
    IF v_payment.method IN ('CREDIT', 'ADJUSTMENT', 'OFFSET') THEN
        v_liquidity_acc := v_partner_acc;
    END IF;
    ...
    VALUES (
        ...
        v_effective_company_id, 
        v_liquidity_acc, 
        CASE WHEN v_payment.method IN ('CREDIT', 'ADJUSTMENT', 'OFFSET') THEN v_contact_id ELSE NULL END,
        CASE WHEN v_is_receipt THEN v_amount ELSE 0 END, 
        CASE WHEN v_is_receipt THEN 0 ELSE v_amount END, 
        COALESCE('Payment: ' || v_ref_val, 'Payment: ' || v_payment.id)
    );
    ...
    IF v_is_receipt AND jsonb_typeof(v_payment.applied_invoices) = 'array' THEN
        FOR v_alloc IN SELECT * FROM jsonb_array_elements(CASE WHEN jsonb_typeof(v_payment.applied_invoices) = 'array' THEN v_payment.applied_invoices ELSE '[]'::jsonb END) LOOP
            SELECT * INTO v_inv_record FROM docs_invoices WHERE id = (v_alloc->>'invoiceId') FOR UPDATE;
            IF FOUND THEN
                SELECT COALESCE(SUM(calc_amount), 0) INTO v_new_amt_paid
                FROM (
                    SELECT p.id,
                        MAX(
                            CASE 
                                WHEN (p.data->>'invoiceId') = v_inv_record.id THEN p.amount
                                ELSE (al->>'amount')::numeric
                            END
                        ) as calc_amount
                    ...
                    WHERE p.status = 'POSTED' AND p.company_id = v_effective_company_id 
                      AND ((p.data->>'invoiceId') = v_inv_record.id OR al->>'invoiceId' = v_inv_record.id)
                    GROUP BY p.id
                ) t;
                ...
                IF v_new_amt_paid > COALESCE(v_inv_record.total, 0) + 1.0 THEN
                    RAISE EXCEPTION 'Double/Over payment detected! Processing this payment would exceed the invoice (%) total.', v_inv_record.invoice_number;
                END IF;
            ...
            END IF;
        END LOOP;
    ELSIF NOT v_is_receipt AND jsonb_typeof(v_payment.applied_bills) = 'array' THEN
        FOR v_alloc IN SELECT * FROM jsonb_array_elements(CASE WHEN jsonb_typeof(v_payment.applied_bills) = 'array' THEN v_payment.applied_bills ELSE '[]'::jsonb END) LOOP
            ...
            IF FOUND THEN
                ...
                FROM (
                    SELECT p.id,
                        MAX(
                            CASE 
                                WHEN (p.data->>'billId') = v_bill_record.id THEN p.amount
                                ELSE (al->>'amount')::numeric
                            END
                        ) as calc_amount
                    FROM docs_payments p
                    ...
                    WHERE p.status = 'POSTED' AND p.company_id = v_effective_company_id 
                      AND ((p.data->>'billId') = v_bill_record.id OR al->>'billId' = v_bill_record.id)
                    GROUP BY p.id
                ) t;
                ...
                IF v_new_amt_paid > COALESCE(v_bill_record.total, 0) + 1.0 THEN
                    RAISE EXCEPTION 'Double/Over payment detected! Processing this payment would exceed the bill (%) total.', v_bill_record.bill_number;
                END IF;
