...
RETURNS JSONB AS $$
    ...
    BEGIN
        ...
        IF EXISTS(SELECT 1 FROM docs_journals WHERE id = v_journal_id) THEN 
            -- Check if cash sale payment needs to be made/re-posted just in case
                ...
                IF NOT EXISTS(SELECT 1 FROM docs_payments WHERE id = v_pay_id) THEN
                    ...
                    VALUES (
                        ...
                        jsonb_build_object(
                            ...
                            'appliedInvoices', jsonb_build_array(jsonb_build_object('invoiceId', p_invoice_id, 'invoiceNumber', v_invoice.invoice_number, 'amount', COALESCE(v_invoice.total, 0), 'remaining', 0))
                        ),
                        NOW()
                    ...
                    UPDATE docs_invoices SET status = 'PAID', data = jsonb_set(data, '{status}', '"PAID"') WHERE id = p_invoice_id;
                END IF;
            END IF;
