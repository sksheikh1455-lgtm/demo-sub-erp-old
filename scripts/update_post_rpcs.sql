...
BEGIN
    ...
    -- Embed lines inside journal to ensure frontend always sees them synchronously
    UPDATE docs_journals 
    SET status = 'POSTED',
        data = jsonb_set(
            jsonb_set(COALESCE(data, '{}'::jsonb), '{status}', '"POSTED"'), 
            '{lines}', 
            COALESCE((SELECT jsonb_agg(jsonb_build_object('id', id, 'accountId', account_id, 'debit', debit, 'credit', credit, 'description', description, 'contactId', contact_id)) FROM docs_journal_lines WHERE journal_id = v_journal_id), '[]'::jsonb)
        ), 
        updated_at = NOW() 
...
BEGIN
    ...
    IF v_is_receipt THEN
        FOR v_alloc IN SELECT jsonb_array_elements(CASE WHEN jsonb_typeof(v_allocs) = 'array' THEN v_allocs ELSE '[]'::jsonb END) LOOP
    ...
    -- Generate Journal as DRAFT first, to avoid premature trigger failure
    INSERT INTO docs_journals (id, company_id, date, journal_type, status, reference_number, data, updated_at)
    VALUES (v_journal_id, v_effective_company_id, v_payment.date, CASE WHEN v_is_receipt OR v_is_refund THEN 'CPAY' ELSE 'VPAY' END, 'DRAFT', COALESCE(v_payment.data->>'reference', v_payment.id), 
        jsonb_build_object('id', v_journal_id, 'date', v_payment.date, 'status', 'DRAFT', 'companyId', v_effective_company_id, 'reference', COALESCE(v_payment.data->>'reference', v_payment.id), 'journalType', CASE WHEN v_is_receipt OR v_is_refund THEN 'CPAY' ELSE 'VPAY' END), NOW())
    ON CONFLICT (id) DO UPDATE SET updated_at = NOW(), status = 'DRAFT', data = EXCLUDED.data;
    ...
    -- Update Journal to POSTED and embed lines so it passes check_journal_balance
    UPDATE docs_journals 
    SET status = 'POSTED',
        data = jsonb_set(
