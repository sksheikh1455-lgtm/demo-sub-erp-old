...
AS $$
            ...
            BEGIN
                ...
                SELECT id INTO v_ar_acc FROM docs_accounts WHERE code IN ('1012','100200','100201','AR') AND company_id = v_effective_company_id LIMIT 1;
                IF v_ar_acc IS NULL THEN SELECT id INTO v_ar_acc FROM docs_accounts WHERE type = 'ASSET' AND name ILIKE '%receivable%' AND company_id = v_effective_company_id LIMIT 1; END IF;
                IF v_ar_acc IS NULL THEN 
                    v_ar_acc := 'acc-ar-' || v_effective_company_id;
                    INSERT INTO docs_accounts (id, company_id, code, name, type) VALUES (v_ar_acc, v_effective_company_id, 'AR', 'Accounts Receivable', 'ASSET') ON CONFLICT DO NOTHING;
                END IF;

                SELECT id INTO v_rev_acc FROM docs_accounts WHERE code IN ('4011', '4000', '400100', 'REVENUE', 'SALES') AND company_id = v_effective_company_id LIMIT 1;
                IF v_rev_acc IS NULL THEN SELECT id INTO v_rev_acc FROM docs_accounts WHERE type = 'REVENUE' AND company_id = v_effective_company_id LIMIT 1; END IF;
                IF v_rev_acc IS NULL THEN
                    v_rev_acc := 'acc-rev-' || v_effective_company_id;
                    INSERT INTO docs_accounts (id, company_id, code, name, type) VALUES (v_rev_acc, v_effective_company_id, 'REV', 'General Revenue', 'REVENUE') ON CONFLICT DO NOTHING;
                END IF;

                SELECT id INTO v_tax_acc FROM docs_accounts WHERE code IN ('2011', '200100', 'TAX_PAYABLE') AND company_id = v_effective_company_id LIMIT 1;
                IF v_tax_acc IS NULL THEN SELECT id INTO v_tax_acc FROM docs_accounts WHERE name ILIKE '%tax%payable%' AND company_id = v_effective_company_id LIMIT 1; END IF;
                IF v_tax_acc IS NULL THEN SELECT id INTO v_tax_acc FROM docs_accounts WHERE type = 'LIABILITY' AND company_id = v_effective_company_id LIMIT 1; END IF;
                IF v_tax_acc IS NULL THEN
                    v_tax_acc := 'acc-tax-' || v_effective_company_id;
                    INSERT INTO docs_accounts (id, company_id, code, name, type) VALUES (v_tax_acc, v_effective_company_id, 'TAX', 'Tax Payable', 'LIABILITY') ON CONFLICT DO NOTHING;
                END IF;
