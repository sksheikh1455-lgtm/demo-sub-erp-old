...
async function run() {
  ...
  console.log("Found 'fiber':", found.length);
  const found2 = all.filter(d => JSON.stringify(d).toLowerCase().includes('optical'));
  console.log("Found 'optical':", found2.length);
  const found3 = all.filter(d => JSON.stringify(d).toLowerCase().includes('solution'));
  console.log("Found 'solution':", found3.length);
}
run();
